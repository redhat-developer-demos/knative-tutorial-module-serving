...
//Varaibles def
ifdef::workshop[]
:service-file: configuration-rev1.remote.yaml
:greeter-image: quay.io/rhdevelopers/knative-tutorial-greeter:quarkus
endif::[]

ifndef::workshop[]
:service-file: configuration-rev1.yaml
:greeter-image: dev.local/rhdevelopers/greeter:0.0.1 #<1>
endif::[]
...
.link:{github-repo}/{configs-and-routes-repo}/config/{service-file}[{service-file}]
...
:url: {github-repo}/{configs-and-routes-repo}/config/{service-file}
:url-alt-text: config/{service-file}
...
It now should return a response like **Hi  greeter => '9be9ccd9a2e3' : 1**
...
ifdef::workshop[]
:service-file: configuration-rev2.remote.yaml
endif::[]

ifndef::workshop[]
:service-file: configuration-rev2.yaml
endif::[]
...
#!/bin/bash
set -eu
SVC_URL=`oc get -n {tutorial-namespace} rt greeter -o yaml | yq read - 'status.url'` 
while true
do
  http --body $SVC_URL
  sleep 3
...
[source,bash,subs="attributes+,+macros"]
----
REVISION_1=`kubectl -n {tutorial-namespace} get rev -l serving.knative.dev/configuration=greeter --sort-by="{.metadata.creationTimestamp}" | awk 'NR==2{print $1}'`
REVISION_2=`kubectl -n {tutorial-namespace} get rev -l serving.knative.dev/configuration=greeter --sort-by="{.metadata.creationTimestamp}" | awk 'NR==3{print $1}'`
----
...
:url-text: route/route_all_rev1.yaml
:revision: $REVISION_1
include::ROOT:partial$crtd-send-traffic.adoc[tag=single]
...
include::ROOT:partial$crtd-send-traffic.adoc[tag=double]
