...
ifndef::workshop[]
:greeter-image:  docker.io/demo/event-greeter:0.0.1 <1>
endif::[]
...
Refer to xref:knative-tutorial-build:ROOT:build.adoc#build-set-env[Build Environment Variables]

[NOTE,subs="macros+"]
====
If you have skipped the xref:knative-tutorial-build:ROOT:build.adoc[Build] or xref:knative-tutorial-build:ROOT:build-templates.adoc[Build Templates] and dont have the container image of `greeter`; then you can update the Knative serving service to use the pre-built greeter image available at `quay.io/rhdevelopers/knative-tutorial-greeter:quarkus`
====
endif::[]
