...
[NOTE,subs="macros+"]
====
If you have skipped the xref:knative-tutorial-build:ROOT:build.adoc[Build] or xref:knative-tutorial-build:ROOT:build-templates.adoc[Build Templates] do not have the container image of `greeter`; then you can update the Knative serving service to use the pre-built greeter image available at `quay.io/rhdevelopers/knative-greeter:quarkus`
====
endif::[]
